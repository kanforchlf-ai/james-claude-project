...
BASE_SIZE_ERAS = [
    ...
    ((7, 1), {'y1': 24, 'y2': 26, 'y3': 27,
              ...
              'ms1': 58, 'ms2': 67}),
    # 2026-08 排區大重組：整個會所名冊重新分掛（hs2 移出 ~90 掛名者到 y2/y3、
    # ms1↔ms2 大互調）；改以「近 4 週最佳主日出席」為基準；合計 223
    ((8, 1), {'y1': 24, 'y2': 21, 'y3': 23,
              'hs1': 21, 'hs2': 17, 'hs3': 19,
              'ms1': 52, 'ms2': 46}),
]
...
# 各小區配搭名單（2026-08 排區重組後歸屬，共 68 位；2026-08-17 黃主業離開移除）
COWORKERS_MAP = {
    'y1':  ['張鈞哲','廖健智','黃珮甄','石真','鍾歆','塗家柔','蔡宜薰','張詠恩','吳均平'],
    'y2':  ['吳秉祈','董宜睿','冉以琳','吳貞儀','陳聿新','鍾安怡','王晨瑄','彭柏愷','趙心樂'],
    'y3':  ['甘順基','沈以恩','朱璟軒','林書丞','王新恩','張若筠','吳汶靜','呂柔蒨','葉倢妤','戴采薰','鍾愛','蔡溢恩'],
    'hs1': ['石錄','陳昱廷','吳秉佳','張韶謙','陳柏安','程琳'],
    'hs2': ['張宏恩','方厚民','黃梓昀','黃梓恆'],
    'hs3': ['陸佳依','呂德鈞','陳信維','陳道璇'],
    'ms1': ['雷嗣益','呂德安','張怡東','于詮暐','戴昀庭','簡若安','汪佩蒂','史以凡','江成聖','吳宇星','莊丹萱','顏惟聆','廖宜人'],
    'ms2': ['呂士成','鍾禹鋒','程旭弘','陸承光','許憶棻','戴采寧','廖智柔','強承安','陳家潔','吳均和','楊媃伊'],
}
